...
class Central2025BacktestTest(unittest.TestCase):
    @classmethod
    ...
    def test_extracts_2025_central_decisive_matches(self) -> None:
        matches = self.read_csv("match_results_2025_central.csv")

        self.assertEqual(88, len(matches))
        self.assertEqual({"2025"}, {row["year"] for row in matches})
        self.assertEqual({"中部赛区"}, {row["region"] for row in matches})
    ...
    def test_predictions_keep_pre_event_cutoff_and_upset_fields(self) -> None:
        predictions = self.read_csv("model_predictions_2025_central.csv")

        self.assertEqual(88, len(predictions))
        self.assertEqual(
            {"rmuc_2025_central_pre_event_v1"},
            {row["model_run_id"] for row in predictions},
        )
        for row in predictions:
            self.assertEqual("2025-05-20", row["data_cutoff"])
            ...
            self.assertIn("feature_cutoff=2025-05-20", row["notes"])

    def test_backtest_summary_is_recorded(self) -> None:
        backtests = self.read_csv("model_backtests_2025_central.csv")
        row = next(
            item
            for item in backtests
            if item["backtest_id"] == "backtest_2025_central_pre_event"
        )

        self.assertEqual("88", row["n_matches"])
        self.assertEqual("2025_central_regional_pre_event_cutoff", row["split_name"])
    ...
    def test_report_is_written(self) -> None:
        report = ROOT / "reports" / "2025_central_backtest.md"
        self.assertTrue(report.exists())
        text = report.read_text(encoding="utf-8")
        self.assertIn("2025 中部赛区", text)
        self.assertIn("严格赛前特征", text)

    def test_database_loads_central_backtest_tables(self) -> None:
        subprocess.run(
        ...
        with sqlite3.connect(DATA_DIR / "robomaster_2026.db") as conn:
            ...
            self.assertIn("match_results_2025_central", tables)
            self.assertIn("model_predictions_2025_central", tables)
            count = conn.execute(
                "SELECT COUNT(*) FROM model_predictions_2025_central"
            ).fetchone()[0]
